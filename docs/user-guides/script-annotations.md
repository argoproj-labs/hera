...
functions to the Parameter's `loader` and `dumper` attributes, which will be used by the Hera Runner (and so requires
the use of "runner" templates).
...
The fields allowed in the `Artifact` annotations are: `name`, `path`, `loader` and `dumper`. You are also able to use
artifact repository types such as `S3Artifact` (which are subclasses of `Artifact`) to first fetch the artifact from
storage and mount it to the container at the inferred path (or your custom path).

## Custom Serialisation

If you are using a type that is not a built in (`int`, `str`, `dict` etc) or a Pydantic `BaseModel`, you can provide
functions to the Artifact's `loader` and `dumper` attributes, which will be used by the Hera Runner (and so requires the
use of "runner" templates). Otherwise, you can set `loader` to an `ArtifactLoader` enum for simple use cases.
