...
from typing import Any, Generator, Iterable, List, Optional, Tuple, Union, get_args
...
def json_to_python(value: Any, annotation: Optional[type] = None) -> Tuple[str, List[str]]:
    """Convert a JSON value to a Python string representation."""
...
def template_to_python(model: Template) -> Tuple[str, List[str]]:
    return (
        f"""\
Container(
    name="{model.name}",
