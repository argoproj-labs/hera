...
def _get_dumper_function(destination: Union[Parameter, Artifact]) -> Callable:
    if isinstance(destination, (Artifact, Parameter)) and destination.dumper is not None:
...
def get_annotated_artifact_value(artifact_annotation: Artifact, func_param_annotation: type) -> Union[Path, Any]:
    ...
    If the artifact is an input, return the  loaded value (json obj, path or string)
    using its ArtifactLoader, unless ArtifactLoader is a Callable, in which case it
