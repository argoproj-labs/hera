...
class Data(TemplateMixin, IOMixin):
    source: Union[m.DataSource, m.ArtifactPaths, _BaseArtifact]
    ...
    def _build_source(self) -> m.DataSource:
        if isinstance(self.source, m.DataSource):
            ...
            return m.DataSource(source=self.source)
        return m.DataSource(source=self.source._build_artifact_paths())
