...
with Workflow("test-env-variables", inputs=[Parameter(name="env-value", value="wf-env-value")]) as wf:
    Task(
        "test-env",
        inspect_envs,
        env=[
            ...
            ConfigMapEnvFrom(name="<your-reference>"),  # Assumes the user has a config map in the k8s cluster
        ],
    )
