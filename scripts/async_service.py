...
class ServiceEndpoint:
    ...
    def __str__(self) -> str:
        ...
        if len(self.params) == 0:
            signature = f"async def {self.name}(self) -> {self.response}:"
        else:
            signature = "async def {name}(self, {params}) -> {ret}:"
        ...
        if len(query_params) > 0:
            params = (
                "{ k: v for k, v in {"
                + ", ".join([f"'{p.field}': {p.name}" for p in query_params])
...
def make_service(service_def: str, endpoints: List[ServiceEndpoint], models_type: str) -> str:
    ...
    result += f"\n\n__all__ = ['Async{models_type.capitalize()}Service']"
    return result
...
if __name__ == "__main__":
    ...
    service_def = service_def.format(
        ...
        module=models_type,
        models_type=f"Async{models_type.capitalize()}",
    ...
    path = Path(__name__).parent / f"src/hera/{models_type}/async_service.py"
